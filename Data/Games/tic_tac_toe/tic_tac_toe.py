...
import itertools
import random
from Data.Speak_All.speak_input import speakIN
...
def main_ttt():
    ...
    def play_move(state, player, block_num):
        if state[int((block_num - 1) / 3)][(block_num - 1) % 3] is ' ':
            ...
            block_num = int(speakIN("Block is not empty, ya blockhead! Choose again: "))
    ...
    def getBestMove(state, player):
        ...
        speak('Possible moves = ' + str(moves))
        print('Move values = ' + str(curr_state_values))
        speak('Move values shown in here')
    ...
    while play_again == 'Y' or play_again == 'y':
        ...
        player_choice = speakIN("Choose which player goes first - X (You - the petty human) or O(The mighty AI): ")
        winner = None
        ...
        while current_state == "Not Done":
            ...
            if current_player_idx == 0:  # Human's turn
                block_choice = int(speakIN("your turn sir! Choose where to place (1 to 9): "))
                play_move(game_state, players[current_player_idx], block_choice)

            else:  # AI's turn
                block_choice = getBestMove(game_state, players[current_player_idx])
                play_move(game_state, players[current_player_idx], block_choice)
                speak("AI plays move: " + str(block_choice))

            print_board(game_state)
            winner, current_state = check_current_state(game_state)
            if winner is not None:
                speak(str(winner) + " won!")
            else:
        ...
        play_again = speakIN('Wanna try again?(Y/N) : ')
